...
test('quality presets lock resolution, AA, AO, streaming and shadow budgets', () => {
  ...
  const shadowFits = Object.values(RENDER_QUALITY_PRESETS).map((preset) => (
    getShadowCameraFit(preset.shadows, sunDirection, -40, 450, 0.5)
  ));

  for (const fit of shadowFits) {
    const horizontalSunLength = Math.hypot(sunDirection.x, sunDirection.z);
    const projectedHeight = 245.5 * horizontalSunLength;

    assert.equal(fit.centerY, 205);
    ...
    assert.ok(fit.halfDepth >= fit.halfWidth * horizontalSunLength + 245.5 * Math.abs(sunDirection.y));
  }
  assert.deepEqual(
