...
`main` 仍是明显的视觉标杆，但它不是纯模型结果，而且严格指令分不高：它没有完整实现新版九宫格/v2 验收契约，还主动加载 `public/assets/enemy/` 并加入敌人和战斗，直接违反题面的禁止项。因此，`main` 作为视觉参考单列，不参与纯模型第一名归属。
...
| ---: | --- | --- | ---: | ---: | ---: | ---: | ---: | --- |
| 标杆 | `main` | GPT 5.6 Sol ultra + 手动调试 | 13.0 | 22.0 | 22.0 | 18.0 | **75.0** | 视觉标杆；严格 v2 契约不完整，且违反敌人/战斗禁止项 |
...
- **main** 使用项目玩家、树木和 VarA–F 草资产，也确实是第三人称；但它主动加载 `/assets/enemy/*`、生成敌人并实现攻击/防御/火球 UI，违反“不得加入敌人、战斗或 NPC”的明确禁止项，所以禁止项 `/3` 为 0。
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `main` | 4.75 | 4.0 | 3.75 | 3.75 | 3.5 | 2.25 | **22.0** |
...
| `main` | **28.8**（27–30） | 34.7 ms | 约 1,388 calls / 12.14M tris | 0 | 原始 FPS 低，但完整可见负载最大 |
...
7 个来源都重新确认生产构建通过；分支测试均通过。

| 来源 | src 文件 / LOC | test 文件 / LOC | 测试 | 生产 JS | 备注 |
| --- | ---: | ---: | ---: | ---: | --- |
| `main` | 50 / 23,827 | 39 / 11,330 | 276/276 | 1,220.58 kB | 代码和视觉系统最完整，但包含题面禁止的战斗系统 |
...
| 视觉产品基线 | `main` | 当前实际画面显著领先 | 移除敌人/战斗、补九宫格与 v2 契约、优化 draw calls |
...
`main` 仍应作为视觉标杆，而不能作为严格指令遵循标杆：其敌人/战斗内容是明确禁止项。若后续希望形成真正公平的性能第二轮，优先修复 Sol-2/GLM shader、Luna Max renderer.info、GPT 5.5 ready 语义，再由统一外部 RAF evaluator 在等价画面下复测。
