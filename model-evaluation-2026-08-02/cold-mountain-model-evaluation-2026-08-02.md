...
`main` 是明显的视觉标杆，总分 **82.5/100**；它包含 GPT 5.6 Sol ultra 的结果和后续手动调试，因此不参与“纯模型第一名”的归属。

五个纯模型分支中：

1. **GPT 5.5 xhigh（61.0）**：宏观地形和材质层次最接近成片，但当前 production preview 在同步构建阶段阻塞超过 60 秒，草模型存在明显青色/黑色 alpha 碎片。
2. **DeepSeek V4 Flash max（60.5）**：代码覆盖和模块边界最好，但树资产归一化失败，大量树木横倒或悬浮，最终帧失真严重。
...
| 性能帧率 | 20 | FPS/帧时间 8、负载效率 5、启动/稳定性 4、指标可信度 3 | 只在可见内容大致等价时直接比较；缺草、缺水、黑洞等造成的高 FPS 必须折价 |
...
|---:|---|---|---:|---:|---:|---:|---|
| 标杆 | `main` | GPT 5.6 Sol ultra + 手动调试 | 34.0 | 35.5 | 13.0 | **82.5** | 视觉通过；严格 v2 契约不完整 |
| 1 | `feat/gpt-5.5-test` | GPT 5.5 xhigh | 30.5 | 25.0 | 5.5 | **61.0** | 未通过 |
| 2 | `codex-ds-flash` | DeepSeek V4 Flash max | 31.5 | 19.0 | 10.0 | **60.5** | 未通过 |
...
GPT 5.5 与 DeepSeek 只差 0.5 分，应视为同一档：前者更接近可看的环境成片，后者更接近完整的系统实现。
...
- **DeepSeek V4 Flash**：湖泊和大地形存在，但树木主轴错误，成片横倒/悬浮并遮挡镜头，这是决定性的视觉失败。
...
| `codex-ds-flash` | 6.0 | 4.5 | 2.5 | 1.0 | 4.0 | 1.0 | **19.0** |
| `gpt-luna` | 3.0 | 2.0 | 1.5 | 1.0 | 1.5 | 1.5 | **10.5** |
| `feat/GLM-5-2` | 3.0 | 1.5 | 0.5 | 1.5 | 1.5 | 0.5 | **8.5** |

DeepSeek 的视觉分低于 Sol，并不是因为系统内容少；恰恰相反，它渲染了大量树木和较完整的世界层。扣分来自这些内容在最终帧中以错误方向覆盖画面，属于“实现存在但结果失真”。

## 6. 性能与帧率

### 当前运行证据

| 分支 | 当前 HUD FPS | Draw calls | Triangles | 启动/稳定等待 | 性能分 /20 | 解释 |
|---|---:|---:|---:|---|---:|---|
| `main` | **42.1**（10 次：41–43） | 1,731 | 17.20 M | 约 60 秒才到完整稳定帧 | 13.0 | 内容最完整；帧率有意义但负载偏重、启动慢 |
| `feat/gpt-5.5-test` | **N/A** | 230¹ | 2.17 M¹ | 当前运行主线程阻塞 >60 秒 | 5.5 | 提交值 71.6 FPS 只有 3 帧/41.9 ms，不是有效 10 秒窗口 |
| `codex-ds-flash` | **53.0**（10 次均为 53） | 470¹ | 21.54 M¹ | 提交的 `readyMs=20615` | 10.0 | 当前均值尚可，但提交快照为 35.48 FPS、p95 91.8 ms、1% low 4 FPS |
...
- GPT 5.5 当前没有取得有效帧率；其同步 `24×24` chunk 构建是本轮最明显的启动性能问题。
...
| `codex-ds-flash` | 9.5 | 9.0 | 7.5 | 5.5 | **31.5** |
...
- 扣分：`src/vegetation/grass.js` 的 `normalizeAsset()` 以包围盒最长轴推断朝向并旋到 +Y；对当前树资产不成立。代码中的启发式看似合理，但最终帧证明其输出错误。
- 判断：适合作为系统架构参考，不适合作为当前视觉基线；必须用已知资产坐标系或离线标准化替代启发式旋转。
...
| 复用系统架构 | `codex-ds-flash` | 模块边界和系统覆盖最完整 | 树/草资产离线归一化、尾延迟、最终帧视觉门禁 |
| 小规模代码继续迭代 | `test/sol-2` | 规模适中、主要数据路径存在 | grass shader、湖面几何、renderer.info |
| 视觉产品基线 | `main` | 当前实际效果远高于纯模型分支 | 补 v2 验收 API、九宫格契约、启动与 draw-call 优化 |
...
如果只看纯模型结果，**GPT 5.5 xhigh 是当前最合理的继续开发起点**，但其 61.0 分并不代表接近完成：加载阻塞和草材质仍是产品级阻断。**DeepSeek V4 Flash max** 仅落后 0.5 分，代码架构甚至更强，但树资产方向错误让最终画面无法交付。**GPT 5.6 Sol xhigh** 的高 FPS 被 shader 缺失污染。**GLM 5.2 xhigh** 展示了“测试很多但最终帧仍失败”的典型风险。**GPT 5.6 Luna medium** 则用最少代码换来了稳定轻量原型，但没有达到题面要求的视觉复杂度。

以实际画面为目标，仍应把 `main` 当作基线：保留其已有地形、材质和生态效果，补齐当前 v2 验收契约，并优先降低首轮稳定时间、draw calls 和三角形负载。
