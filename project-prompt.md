...
```json
{
  ...
  "mainRiver": {
    ...
    "waterLevels": [
      3.2, 3.05, 2.9, 2.72, 2.55, 2.38, 2.2, 2.05, 1.88, 1.72, 1.6
    ],
    "confluences": [
...
- 所有树木和草模型的静态根到顶主轴必须对齐世界 `+Y`；不得沿地形 normal 倾倒，随机方向只能绕世界 `Y` 轴旋转；
...
不得假定 GLB 的原始 up axis 已经符合运行时约定。每个树源模型加载后必须只做一次 asset-space 归一化：把树根到树冠中心的主轴修正为模型局部 `+Y`，并让根部接触平面位于局部 `y=0`。修正应烘入共享 geometry 或单一 normalization root，不能在 clone、primitive、LOD 和 instance 各重复旋转一次。归一化后的实例只允许绕世界 `Y` 轴随机 yaw；禁止按地形法线设置 pitch/roll，禁止用 `setFromUnitVectors(UP, terrainNormal)` 或等价方式让整棵树贴坡倾倒。
...
VarA–VarF 的每个 LOD 都必须执行一次与树木相同的 asset-space 朝向归一化：草簇根部中心到叶尖中心的主轴映射到模型局部 `+Y`，根部接触平面映射到局部 `y=0`。LOD0/1/2 不得各自保留不同的 90°/180° 源轴修正，也不得在实例阶段重复补偿。静态草实例只能绕世界 `Y` 轴 yaw，不得贴合坡面 normal；cross-card 或 billboard 只能绕 `Y` 面向相机，不能随 camera pitch 向后躺倒。
...
- 相机平滑跟随，不能穿入地形；
- 角色和相机不能越出地图边界；
- 可选调试飞行：Option 上升、Ctrl 下降，便于评测远景，但不能破坏正常地面移动。
...
- 静态实例的根到顶主轴垂直对齐世界 `+Y`，只允许绕 `Y` 随机 yaw，不跟随地形 normal 倾斜；
...
- 树根到树冠的静态主轴垂直对齐世界 `+Y`，实例不得产生 pitch/roll；
...
Option      按住上升；升空后水平移动速度为对应地面速度的 10 倍
Ctrl        下降，最低回到地面
鼠标左拖    环绕相机
滚轮        调整相机距离
...
- 步行速度约 4–7 单位/秒；
- Option 使角色升空后，WASD 水平移动速度必须为对应地面速度（普通或 Shift 加速）的 `10×`；降回地面后立即恢复正常速度；
- 转向使用阻尼或角度插值；
- 最大可走坡度约 42–50 度；
...
```text
C_asset: source biological-up → local (0,1,0)，source root plane → local y=0
M_instance = T(x, terrainHeight + rootOffset, z) × R_y(yaw) × S(scale)
```

`C_asset` 只应用一次；`M_instance` 不包含 X/Z 旋转或 terrain-normal quaternion。对任意坡度、变体、LOD 和随机 yaw，令 `u = normalize(mat3(M_instance) × vec3(0,1,0))`，必须满足 `dot(u, vec3(0,1,0)) >= 0.99985`，即静态偏离世界竖直方向不超过约 `1°`。模型自身叶片造型弯曲不算实例倾斜。风动只能使根部以上顶点临时弯曲，根部顶点位移必须为 0；LOD cross-fade 前后根部位置和 up axis 不变。
...
树木沿用第 20.2 节的 `C_asset` 与 `M_instance` 契约以及 `dot(up,+Y) >= 0.99985` 容差。地形 normal 只用于判断坡度和是否接受候选，不用于旋转树实例。允许非均匀尺度和绕 `Y` 的 yaw，但不能引入 X/Z tilt。远树 impostor 或 billboard 的竖直轴同样固定为世界 `+Y`，只能绕 `Y` 朝向相机；从详细模型切换到远景代理时不能突然横躺、翻转或改变根部高度。
...
16. tree_01–04、tree_spawn 与 VarA–VarF/LOD0–2 的 asset-space 修正都把根到顶主轴映射到局部 `+Y`，根部接触面位于局部 `y=0`；
17. 在平地和多个合成斜坡 normal 上生成树草实例，分解实例矩阵所得 up vector 均满足 `dot(up,+Y) >= 0.99985`；
18. 改变随机 yaw、尺度和 LOD 不改变 up vector 或根部世界高度，且放置代码不使用 terrain normal 构造实例 quaternion；
19. 草风动在根部的位移为 0，cross-card、billboard 和 tree impostor 的竖直轴保持世界 `+Y`。
...
- 树木或草模型大面积横躺、倒置，或整批沿地形 normal 倾斜而非保持世界 `+Y`：产品门槛失败；
...
13. 从 live scene 抽样详细树、远树代理、Ribbon grass 各变体/LOD 的实际 `matrixWorld`/instance matrix 和根部 bounds；确认修正后的根到顶主轴与世界 `+Y` 点积至少 `0.99985`、根部贴地，并在斜坡、yaw 和 LOD 切换时保持不变。
...
- [ ] 树木、Ribbon grass、cross-card 和远树代理的根到顶主轴垂直对齐世界 `+Y`，随机变换只有 `Y` 轴 yaw，不沿地形 normal 倾斜；
