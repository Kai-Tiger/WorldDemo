...
- English: Every surface vertex provides the same twelve attributes: `waterDepth`, `shoreDistanceMeters`, `shoreFoamMask`, `flowUv`, `flowDirection`, `junctionFlowDirection`, `flowSpeed`, `riverInfluence`, `rapidMask`, `junctionMask`, `disturbanceMask`, and `reflectionTier`. `shoreFoamMask` is zero only on the five river-to-lake transition rows and one elsewhere, so it removes false lake-shore foam at water-to-water junctions without changing coverage, vertex waves, or hydraulic river foam. Lake-only values use neutral river defaults. `reflectionTier` is `0` for environment, `0.5` for probe, and `1` for alpine planar reflection.
- 中文：所有水面顶点都提供完全相同的十二项属性：`waterDepth`、`shoreDistanceMeters`、`shoreFoamMask`、`flowUv`、`flowDirection`、`junctionFlowDirection`、`flowSpeed`、`riverInfluence`、`rapidMask`、`junctionMask`、`disturbanceMask` 和 `reflectionTier`。`shoreFoamMask` 仅在五排河湖过渡面上为零，其余位置为一，用于移除水水接口处错误的湖岸泡沫，同时不改变 coverage、顶点波浪或河流水力泡沫。纯湖泊区域使用中性的河流属性默认值；`reflectionTier` 的 `0` 表示 environment、`0.5` 表示 probe、`1` 表示高山湖 planar reflection。
